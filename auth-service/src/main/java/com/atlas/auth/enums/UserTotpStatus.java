...
@Getter
public enum UserTotpStatus implements MybatisBaseEnum<String> {
    ...
    // 构造函数
    UserTotpStatus(String code, String description) {
        this.code = code;
        this.description = description;
    }

    @JsonCreator
    public static UserTotpStatus fromString(String value) {
        if (value == null || value.trim().isEmpty()) {
            return null;
        }
        try {
            // 核心：去除空格并转为大写后再匹配
            return UserTotpStatus.valueOf(value.trim().toUpperCase());
