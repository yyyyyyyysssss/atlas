...
public class OAuth2ClientApplicationController {
    ...
    @PostMapping("/save")
    public Result<OAuth2ClientApplicationCreateVO> save(@RequestBody @Validated OAuth2ClientApplicationSaveDTO saveDTO){
        OAuth2ClientApplicationCreateVO createVO = oAuth2ApplicationFacadeService.save(saveDTO);
        return ResultGenerator.ok(createVO);
    }

    @GetMapping("/{id}")
    public Result<OAuth2ClientApplicationVO> getDetail(@PathVariable("id") Long id){
        OAuth2ClientApplicationVO vo = oAuth2ApplicationFacadeService.getApplicationDetail(id);
        return ResultGenerator.ok(vo);
    }

    @GetMapping("/page")
    public Result<PageInfo<OAuth2ClientApplicationVO>> getPage(OAuth2ClientApplicationQueryDTO queryDTO) {
