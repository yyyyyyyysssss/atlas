...
授权码模式是 OAuth2 中最安全、最常用的认证授权模式，适用于拥有后端服务器的应用（如 Web 应用或服务端的移动应用代理）。
...
```bash
curl --location --request GET 'https://atlas.ys0921.sbs:4443/api/auth/oauth2/authorize' \
...
```bash
curl --location --request POST 'https://atlas.ys0921.sbs:4443/api/auth/oauth2/token' \
...
```bash
curl --location 'https://atlas.ys0921.sbs:4443/api/auth/oauth2/userinfo' \
