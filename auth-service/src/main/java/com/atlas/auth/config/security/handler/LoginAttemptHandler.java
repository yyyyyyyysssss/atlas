package com.atlas.auth.config.security.handler;
...
public class LoginAttemptHandler {
    ...
    public LoginAttemptHandler(RedisHelper redisHelper){
        this(redisHelper, 5, 15 * 60 * 1000L, 30 * 60 * 1000L);
    }

    public LoginAttemptHandler(RedisHelper redisHelper, int maxAttempt, long windowMs, long lockMs) {
        if(redisHelper == null) {
